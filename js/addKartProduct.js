...
const createSearchProductCard = (product) => {
  ...
  <div class="col-md-6">
    <div class="products_kart">
      ...
      <div><strong class="fs-3">₹${formatIndianRupee(afterDiscontPrice)}</strong> <del style="color:#878787">₹${formatIndianRupee(product.price)}</del> <span style="color:#388e3c">${discont}% off</span> <span style="color:grey;font-size:10px;padding:0px 4px;border-radius:50%;border:1px solid grey;">?</span></div>
      ...
      <div style="background-color:#f8f8f8;padding:20px 10px;width:70%;"><span style="color:grey;font-size:15px;font-weight:500;">Warranty</span><span style="font-size:14px;"> &nbsp &nbsp Covers Manufacturing Defects <a href="#">Know More</a></span></div>
          ...
          <div style="border:none;border-bottom:2px solid #2874f0;">
            <form autocomplete="off" style="display:flex;align-items:center;width:100%;">
              <div style="position:relative;flex-grow:1;">
                <svg width="16" height="16" viewBox="0 0 9 12" xmlns="http://www.w3.org/2000/svg" style="position:absolute;top:50%;left:10px;transform:translateY(-50%);"><path fill="#2874f0" d="M4.2 5.7c-.828 0-1.5-.672-1.5-1.5 0-.398.158-.78.44-1.06.28-.282.662-.44 1.06-.44.828 0 1.5.672 1.5 1.5 0 .398-.158.78-.44 1.06-.28.282-.662.44-1.06.44zm0-5.7C1.88 0 0 1.88 0 4.2 0 7.35 4.2 12 4.2 12s4.2-4.65 4.2-7.8C8.4 1.88 6.52 0 4.2 0z" fill-rule="evenodd"></path></svg>
              ...
              <button style="padding:10px 0;background-color:#f8f8f8;color:#2874f0;border:none;margin-left:10px;cursor:pointer;font-weight:550;font-size:15px;position:relative;">Check</button>
            </form>
          </div>
        </div>
...
const createSearchSimilarProducts = (product) => {
  const discont = Math.floor((product.rating * parseInt(product.price.toString().slice(0, 2))) / 10);
  const afterDiscontPrice = Math.round(((100 - discont) * product.price) / 100);
  return `
  <style>.productsSimi div{font-size:14px}.productsSimi img:hover{transform:scale(1.1)}.productsSimi{font-size:medium;display:flex;flex-direction:column;align-items:center}.rating{background-color:green;width:fit-content;padding:0 16px;border-radius:5px;color:#fff}.productsSimi .product_Img{height:150px}.productsSimi img{padding:1px;width:100%;height:100%;object-fit:contain}</style>
...
const searchFetch = (products) => {
  document.getElementById("addkart").innerHTML = products.map(createSearchProductCard).join("");
};

const searchFetchSimi = (products) => {
  document.getElementById("similarProducts").innerHTML = products.map(createSearchSimilarProducts).join("");
};
...
fetch("https://raw.githubusercontent.com/csathnere/APIs/main/json-ec/product.json")
  ...
  .then((data) => {
    ...
    searchFetch(data.filter((product) => product.name == query));
    setInterval(() => searchFetchSimi(data.filter((product) => product.category == simiCategory).slice(0, 12)), 1500);
  })
